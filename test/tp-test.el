...
(ert-deftest include-test ()
    (save-window-excursion
      (let* ((test-file "test-include.typ")
	    (main-file "test.typ"))
	(find-file test-file)
	(should test-file)		 ; test file present
...
(ert-deftest slide-test ()
    (save-window-excursion
      (let ((test-file "test.typ")
	    (typst-preview-preview-mode "slide"))
	(find-file test-file)
	(should test-file)		 ; test file present
	(setq typst-preview--master-file test-file) ; prevent asking for main file
	(typst-preview-mode)
